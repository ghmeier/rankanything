...
func (a *App) registerRankingRoutes(mux *http.ServeMux) {
	...
	mux.Handle("POST /r/{uuid}/items/{itemID}/unrank", a.RequireRankingAccess(http.HandlerFunc(a.handleUnrankItem)))
	mux.Handle("POST /r/{uuid}/tiers", a.RequireRankingAccess(http.HandlerFunc(a.handleAddTier)))
	mux.Handle("POST /r/{uuid}/tiers/reorder", a.RequireRankingAccess(http.HandlerFunc(a.handleReorderTiers)))
	...
	mux.Handle("POST /r/{uuid}/tiers/{tierID}/items/reorder", a.RequireRankingAccess(http.HandlerFunc(a.handleReorderTierItems)))
	mux.Handle("POST /r/{uuid}/publish", a.RequireRankingAccess(http.HandlerFunc(a.handlePublishVersion)))
